...
<display:table
	...
	<display:column titleKey="box.name">
				...
				<jstl:if test="${fn:length(row.messages) lt 1}">
				  		<jstl:out value="${row.name}" /> 
				</jstl:if>
	</display:column>
	
<%-- Columna para mostrar link para ver los messages de una box--%>
	<display:column   titleKey="box.numMessages" sortable="true">
  				<jstl:out value="${fn:length(row.messages)}" /> 
	</display:column>
...
<input type="button" name = "createBox" value ="<spring:message code = "box.create"/>" onclick="window.location.href = 'box/create.do'">
